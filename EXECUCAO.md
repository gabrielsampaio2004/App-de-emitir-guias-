...
Um por sessão, na ordem sugerida. Sessão nova a cada etapa, sempre depois de
`git pull`. Não emende duas etapas na mesma conversa.

### Etapa A — envio duplicado

```
Leia CLAUDE.md, HANDOFF.md e EXECUCAO.md. Execute a etapa A do EXECUCAO.md —
só ela.

Reproduza o envio duplicado antes de corrigir qualquer coisa. Se não
conseguir reproduzir, me diga em vez de corrigir no escuro. A decisão de
pular SENDING já está tomada no documento; o que fica com você é COMO tornar
a linha presa visível para um humano — escolha e documente, como foi feito na
seção 4 do HANDOFF.

Resolva também o idempotencyKey que o provider promete e o meta-cloud ignora.

No fim: rode o aceite e me mostre a saída dos comandos, separando o que você
executou do que apenas leu. Atualize o HANDOFF.md — inclusive tirando este
bug da seção 2.
```

Precisa de Postgres **e Redis** rodando. Confira com `redis-cli ping` antes.

### Etapa C — testes

```
Leia CLAUDE.md, HANDOFF.md e EXECUCAO.md. Execute a etapa C do EXECUCAO.md —
só ela.

Use node:test nativo. Nenhuma dependência nova: isso é condição da
autorização registrada no documento. Os cinco casos do parseFilename da seção
3 do HANDOFF são obrigatórios; as três suítes extras listadas na etapa C
também.

Não escreva teste de integração que dependa de Postgres ou Redis sem me
perguntar antes.

No fim: rode npm test e me mostre a saída. Atualize o HANDOFF.md.
```

### Etapa D — nome do template configurável

```
Leia CLAUDE.md, HANDOFF.md e EXECUCAO.md. Execute a etapa D do EXECUCAO.md —
só ela. É curta.

Mantenha "envio_guia_fiscal" como default e documente a variável nova no
.env.example. Não encoste na decisão travada de categoria UTILITY.

No fim: typecheck e build, e atualize o HANDOFF.md.
```

### Etapa B — autenticação (a maior)

```
Leia CLAUDE.md, HANDOFF.md e EXECUCAO.md. Execute a etapa B do EXECUCAO.md —
só ela.

Antes de escrever código, me diga como pretende resolver o conflito entre o
model User que já existe no schema e o schema que o better-auth quer gerar.
Não duplique usuário e não apague o model atual sem entender quem depende
dele.

Trate o multi-tenant como segurança, não cosmética: hoje sete arquivos
assumem que só existe um tenant, e sem sessão um escritório enxerga guia de
outro.

Esta etapa pode não caber numa sessão. Se acabar o fôlego, anote no
HANDOFF.md exatamente onde parou antes de encerrar.

No fim: me mostre a saída do aceite. Atualize o HANDOFF.md.
```

### Etapa E — erros que não chegam na tela

```
Leia CLAUDE.md, HANDOFF.md e EXECUCAO.md. Execute a etapa E do EXECUCAO.md —
só ela.

Mantenha o mínimo possível no cliente. Cubra pelo menos: documento já
agendado, cliente desativado, PDF duplicado e arquivo que não é PDF.

Teste no navegador de verdade, não só por tipo. No fim me mostre a saída e
atualize o HANDOFF.md.
```

### Etapa F — ponta a ponta com a Meta

Só na máquina com o `.env` real, e só depois do template aprovado.

```
Leia CLAUDE.md, HANDOFF.md e EXECUCAO.md. Vamos fechar a etapa F.

As credenciais estão no .env local. Não as imprima, não as commite, não edite
o .env.example com valor real.

NÃO SIMULE NADA. Se algo falhar, me mostre o erro real. Um envio relatado
como bem-sucedido sem ter acontecido é o pior desfecho possível aqui.

Me dê os comandos para eu rodar e vá interpretando as saídas comigo.
```

### Conferir estado, a qualquer momento

```
Leia CLAUDE.md, HANDOFF.md e EXECUCAO.md e me diga, sem alterar nada: em que
pé está o projeto, quais etapas do EXECUCAO.md ainda estão abertas, e se
encontra alguma contradição entre os três documentos ou entre eles e o
código.
```
