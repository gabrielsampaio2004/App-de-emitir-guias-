...
-- AuditLog é append-only (CLAUDE.md): nenhum código deve alterar ou apagar
-- uma linha desta tabela. Isso é reforçado no banco, não só por convenção —
-- REVOKE aqui não tem efeito sobre roles com atributo SUPERUSER (o Postgres
-- ignora GRANT/REVOKE para superusers); em desenvolvimento local, onde a
-- aplicação normalmente conecta como "postgres" (superuser), esta trava fica
-- sem efeito observável. Em produção, a role usada pela aplicação DEVE ser
-- uma role comum (sem SUPERUSER) para esta linha realmente proteger algo.
