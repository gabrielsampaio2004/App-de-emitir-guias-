...
-- CreateIndex
CREATE INDEX "Consent_clientId_status_revokedAt_idx" ON "Consent"("clientId", "status", "revokedAt");
...
-- ---------------------------------------------------------------
-- AuditLog é append-only — regra inviolável do CLAUDE.md
-- ---------------------------------------------------------------

REVOKE UPDATE, DELETE ON "AuditLog" FROM PUBLIC;

-- O REVOKE acima é o que o CLAUDE.md manda, e fica. Mas sozinho ele não entrega
-- a regra: no Postgres o DONO da tabela ignora privilégios de GRANT/REVOKE, e o
-- .env.example conecta com o mesmo usuário que cria o schema. Na prática o
-- UPDATE e o DELETE continuariam passando. Os gatilhos abaixo valem para todo
-- mundo, dono incluído, e é o que de fato torna a tabela append-only.
CREATE OR REPLACE FUNCTION "auditlog_append_only"() RETURNS trigger AS $$
BEGIN
  RAISE EXCEPTION 'AuditLog é append-only: % não é permitido', TG_OP
...
-- TRUNCATE não dispara gatilho de linha; precisa do seu próprio.
CREATE TRIGGER "auditlog_no_truncate"
  BEFORE TRUNCATE ON "AuditLog"
  FOR EACH STATEMENT EXECUTE FUNCTION "auditlog_append_only"();

-- ---------------------------------------------------------------
-- Consent: `status` é a fonte de verdade única
-- ---------------------------------------------------------------

-- Antes disto o schema aceitava status = 'GRANTED' com revokedAt preenchido, e
-- o dispatcher, que só olhava o status, enviaria para quem já tinha revogado.
-- O banco passa a recusar o par contraditório.
ALTER TABLE "Consent" ADD CONSTRAINT "Consent_revokedAt_matches_status"
